...
The minimal repair gives passive effects an exactly-once cancellation cleanup, checks auth-effect ownership again before setup/unlock, cancels the complete recovery effect group on retry, and carries the current root recovery owner through every roster/active/merge continuation. The same two focused tests then passed, followed by app `88/88`, Task 8 core `138/138`, and the browser matrix `14/14` twice. That repair is commit `759d81bd1bf9b85e437552f2ffaa8737792559ab` (`fix(child): close teacher recovery cancellation races`).

The next independent pass confirmed both original P1 paths but found that a superseding PIN request used the old request's cancellation callback to clear the new request's DOM input. Its focused regression failed `1 !== 0` before production change. The cancellation cleanup now always blanks the superseded closure PIN but clears DOM input only for reset/destruction, leaving final DOM cleanup to the current keyed auth effect. The regression then passed. Additional deterministic tests hold and supersede a roster result plus two parallel active-conversation results; neither old continuation can start child reads, merge, persist, or leave the authorized recovery state. This repair is commit `018e8c72d630ed523526cc8abd408a64f5c097b4` (`fix(child): preserve current pin across auth supersede`).

The final independent review returned **GO with no P0/P1** after rerunning the two focused race tests (`2/2`), Task 8 core (`138/138`), syntax checks, and diff checks. It confirmed reset/destruction share the same immediate `cancelAllEffects` cancellation cleanup; only reset has a dedicated delayed-status test, so the missing duplicate destroy row remains a P2 evidence limitation rather than an unverified production branch. Task 8 is approved for the Task 9 start boundary.
